...
export default function Tournament() {
  ...
  const n = useCallback((slotId) => {
    ...
    if (slotId === 'SF1W') {
      const w = semiWinner('SF1', partidos)
      return w ? (slots[w]?.nombre || w) : 'Gan. SF1'
    }
    if (slotId === 'SF2W') {
      const w = semiWinner('SF2', partidos)
      return w ? (slots[w]?.nombre || w) : 'Gan. SF2'
    }
    return slotId
  ...
  const saveSlot = async (slot, nombre, bandera) => {
    ...
    try {
      const params = new URLSearchParams({ method:'torneo', action:'updateSlot', slot, nombre, bandera })
  ...
  const savePartido = async (id, data) => {
    ...
    try {
      const params = new URLSearchParams({
        ...
        visitanteScore: updated.visitanteScore || '',
        penales:        updated.penales        ? 'true' : 'false',
        penalesGanador: updated.penalesGanador || '',
